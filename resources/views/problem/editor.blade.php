...
@php
    ...
    $submittedCode = (filled($preferable_compiler['submission']) && (!$contest_mode || $contest->rule != 5) ) ? $preferable_compiler['submission']->solution : '';
...
<body>
    ...
    <script>
                ...
                headers: {
                    ...
                    if(ret.ret == 200){
                        ['description', 'input', 'output', 'note'].forEach(fieldKey => {
                            $(`div[data-problem-section="${fieldKey}"]`).addClass('d-none');
                            $(`div[data-problem-section="${fieldKey}"] > div`).html('');
                            if(ret.data[fieldKey]) {
                                $(`div[data-problem-section="${fieldKey}"]`).removeClass('d-none');
        ...
        $( "#submitBtn" ).click(function() {
            ...
            $.ajax({
                type: 'POST',
                url: "{{$contest_mode ? route('ajax.contest.solution.judge') : route('ajax.problem.solution.judge')}}",
                data: {
                    lang: chosen_lang,
                    pid:"{{$problem->pid}}",
                    pcode:"{{$problem->pcode}}",
